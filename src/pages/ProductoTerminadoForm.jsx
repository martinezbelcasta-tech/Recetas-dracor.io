                ...
                {results.map(item => {
                  ...
                  return (
                    <button key={item.codigo} onClick={() => { onSelect(item); onClose() }}
                      ...
                      <span className="text-sm text-gray-800 flex-1 group-hover:text-blue-900 leading-snug">{item.nombre}</span>
                      <span className={`text-xs px-2 py-0.5 rounded-full shrink-0 font-medium ${badge.cls}`}>{badge.label}</span>
                    </button>
                  )
                })}
              </div>
...
function autoCodigo(nombre) {
  ...
  return 'PT-' + words.map(w => w[0]).join('').slice(0, 5)
}
...
function Toggle({ value, onChange, label }) {
  return (
    <button type="button" onClick={() => onChange(!value)} className="flex items-center gap-2.5 group">
      <div className={`relative w-10 h-5 rounded-full transition-colors ${value ? 'bg-emerald-600' : 'bg-gray-200'}`}>
...
function PesoField({ label, hint, value, onValue, unit, onUnit }) {
  return (
    <div>
      ...
      <div className="flex gap-2">
        <input type="number" min="0" step="0.001" value={value} onChange={e => onValue(e.target.value)}
        ...
        <select value={unit} onChange={e => onUnit(e.target.value)}
          className="w-16 border border-gray-300 rounded-lg px-2 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-emerald-500">
...
export default function ProductoTerminadoForm({ initial, onSave, onCancel, saving }) {
  const [form, setForm] = useState(() => initial ? { ...initial } : {
    ...
    peso_bruto: '', peso_bruto_unidad: 'kg',
    ...
    <div className="p-8">
      ...
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-2xl font-bold text-gray-900">
              {initial ? 'Editar Producto Terminado' : 'Nueva Receta Producto Terminado'}
            </h2>
            <p className="text-gray-400 text-sm mt-0.5">Semiterminados + materiales de empaque con sus unidades y cantidades</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          ...
          <button onClick={() => canSave && !saving && onSave(form)} disabled={!canSave || saving}
            className="px-5 py-2.5 text-sm font-semibold bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 disabled:opacity-40 disabled:cursor-not-allowed transition-colors shadow-sm shadow-emerald-200 flex items-center gap-2">
        ...
        <div className="col-span-3 bg-white rounded-xl border border-gray-200 shadow-sm p-6 space-y-5">
          ...
          <div className="max-w-xs">
            ...
            <input value={form.codigo}
              ...
              className="w-full border border-gray-300 rounded-lg px-3 py-2.5 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-emerald-500"
              placeholder="PT-..." />
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <PesoField
              ...
              onValue={v => set('peso_neto', v)}
              unit={form.peso_neto_unidad}
              onUnit={v => set('peso_neto_unidad', v)}
            />
            <PesoField
              label="Peso Bruto"
              hint="(con empaque)"
              value={form.peso_bruto}
              onValue={v => set('peso_bruto', v)}
              unit={form.peso_bruto_unidad}
              onUnit={v => set('peso_bruto_unidad', v)}
            />
          </div>
        ...
        <div className="col-span-2 bg-white rounded-xl border border-gray-200 shadow-sm p-6">
          ...
          <label className="cursor-pointer block">
            {form.foto_preview ? (
              ...
              <div className="h-52 border-2 border-dashed border-gray-300 rounded-xl flex flex-col items-center justify-center gap-3 hover:border-emerald-400 hover:bg-emerald-50/20 transition-colors">
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-gray-400 mt-0.5">
              Semiterminados y materiales de empaque · ingresa unidad y cantidad por componente
            </p>
          </div>
          ...
          <table className="w-full">
            <thead className="sticky top-0 z-10">
              <tr className="bg-gray-50 border-b border-gray-200 text-xs font-semibold text-gray-500 uppercase tracking-wider">
                ...
                <th className="text-left px-4 py-3 w-40">Código</th>
                <th className="text-center px-4 py-3 w-32">Unidad</th>
                ...
                <th className="text-left px-4 py-3">
                  <div className="flex items-center gap-2">
                    ...
                    <button type="button" onClick={fillAllUbi}
                      className="text-xs font-normal text-emerald-500 hover:text-emerald-700 border border-emerald-200 hover:border-emerald-400 hover:bg-emerald-50 rounded px-1.5 py-0.5 transition-colors whitespace-nowrap">
                      ↓ Igualar todas
                    </button>
                  </div>
                </th>
            ...
            <tbody className="divide-y divide-gray-50">
              {form.items.map((item, idx) => {
                const isSpecial = SPECIAL_CODES.has(item.comp_codigo)
                const badge     = item.comp_codigo ? getCategoriaBadge(item.comp_codigo) : null
                ...
                return (
                  <tr key={item.id}
                    ...
                    <td className="px-4 py-1.5">
                      <PickerField
                        value={item.comp_nombre}
                        placeholder="Click para buscar componente…"
                        onOpen={() => setModal({ type: 'comp', itemId: item.id })}
                        onClear={() => updateItem(item.id, { comp_codigo: '', comp_nombre: '' })}
                      />
                    </td>
                    ...
                    <td className="px-4 py-1.5 text-center">
                      {isSpecial ? (
                        ...
                        <select value={item.unidad}
                          ...
                          className="border border-gray-300 rounded-lg px-2 py-1 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-emerald-500 w-full">
                          {UNIDADES_PT.map(u => <option key={u}>{u}</option>)}
                        </select>
                      )}
                    </td>
                    ...
                    <td className="px-4 py-1.5">
                      {isSpecial ? (
                        ...
                        <input type="number" min="0" step="0.0001" value={item.cantidad}
                          ...
                          className="w-full border border-gray-300 rounded-lg px-3 py-1.5 text-sm text-right focus:outline-none focus:ring-2 focus:ring-emerald-500"
                          placeholder="0" />
                      )}
                    </td>
        ...
        <div className="px-6 py-3.5 bg-gray-50 border-t border-gray-100">
          <button onClick={addItem}
            className="text-emerald-600 hover:text-emerald-700 text-sm font-medium flex items-center gap-1.5 transition-colors">
      ...
      {modal?.type === 'ubi' && (
        <SearchModal title="Buscar Ubicación de Salida" data={[...ubiExtras, ...UBI_LIST]}
          onSelect={u => updateItem(modal.itemId, { ubi_codigo: u.codigo, ubi_nombre: u.nombre })}
          onClose={() => setModal(null)} />
      )}
    </div>
  )
